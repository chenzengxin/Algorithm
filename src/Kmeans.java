...
public class Kmeans {
	...
	public void setKmeansInput(String path) {
		try {
			...
			while ((str = br.readLine()) != null) {
				strArray = str.split(",");//"1,2" ===>{"1","2"}
				dimension = strArray.length;// 坐标点维数
				Node node = new Node();//顶点
				for (int i = 0; i < dimension; i++) {
					node.attributes[i] = Double.parseDouble(strArray[i]);//将字符串转换成双精度
				}
				arraylist.add(node);//保存在arraylist里
			}
			br.close();//关闭文件读取
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void chooseCentroid(Queue<NodeComparator> queue) {
		centroidList = new ArrayList<Node>();//质心列表
		boolean flag = false;//用于初始加入nclist中最远的两个顶点
		while (!queue.isEmpty()) {
